                ...
                <div v-for="(w, key) in schedule" :key="key">
                  <div v-if="day.date === key + 1">
                    <div v-for="(obj, idx) in w.pop()" :key="idx">
                      <!-- 업데이트 모달 링크 수정 예정(######) -->
                      <a href="#" @click="updateModal(day); assignSchedule(obj.id, obj.memberId, obj.memberName, obj.pt)">
                        {{ obj.memberName }}, {{ obj.pt }}
                      </a>
                      <button type="button" @click="deleteSchedule(obj.id, obj.memberId)" class="fa-solid fa-xmark"></button>
                      <br/>
                    </div>
                  </div>
                </div>
...
// 이전 월로 이동
// TODO: 월 이동 버튼에서 현재 월(currentMonth)을 /api/schedule/getList 의 키값으로 조회하여 schedule 의 값을 해당 월 값으로 치환.
const previousMonth = () => {
  if (month.value === 0) {
    year.value--;
    month.value = 11;
